...
int ring_init(ring_buffer_t *pRingBuf, int size)
{
	...
	pRingBuf->buf = (unsigned char *)MeMalloc(size,__FILE__,__LINE__);
...
void PacketBufferReset(Packet_buffer_t * PacketBuff)
{
...
unsigned int ring_used_size(ring_buffer_t *pRingBuf)
{
	...
	if(pRingBuf->dataStart > pRingBuf->dataEnd)
	{
		//printf("=========================start =%d ,end =%d \r\n",pRingBuf->dataStart,pRingBuf->dataEnd);
		//fflush(stdout);
		return (pRingBuf->bufLen - pRingBuf->dataStart) + pRingBuf->dataEnd;
	}
...
int ring_pop_n(ring_buffer_t *pRingBuf, unsigned char *pBuf, unsigned int len,unsigned int BuffSize)
{
	...
	{
	       printf("size fail len =%d ,buffsize =%d. (ring_used_size(pRingBuf) < len) : %d \r\n",len,BuffSize, (ring_used_size(pRingBuf)));
		return -1;
	}
#if 0
	if(pRingBuf->dataStart < pRingBuf->dataEnd)
	{
		memmove(pBuf,pRingBuf->buf+pRingBuf->dataStart,len);
	}
	else
	{
		int ringBufRleft = pRingBuf->bufLen-pRingBuf->dataStart;
		if(len <= ringBufRleft)
		{
			memmove(pBuf,pRingBuf->buf+pRingBuf->dataStart,len);	
		}
		...
		{
			memmove(pBuf,pRingBuf->buf+pRingBuf->dataStart,ringBufRleft);
			memmove(pBuf+ringBufRleft,pRingBuf->buf,len-ringBufRleft);
		}
	}
#else	
	memmove(pBuf,pRingBuf->buf+pRingBuf->dataStart,len);
...
int ring_free_size_ex(ring_buffer_t *pRingBuf)
{
	...
	if(pRingBuf->dataStart > pRingBuf->dataEnd)
	{
		return pRingBuf->dataStart - pRingBuf->dataEnd - 1;
	}
	else
	{
		return pRingBuf->bufLen - pRingBuf->dataEnd;
...
int PacketBuffer_init(Packet_buffer_t * PacketBuff , unsigned int size)
{
	...
	PacketBuff->BufferAddr = (unsigned char *)MeMalloc(size,__FILE__,__LINE__);
