...
int KCFTrackerEx::KCFTrackerUpdate(Mat p_mtImageMat)
{
	...
	for (int i = 0; i < KCF_MAX_NUM; i++)
	{
		...
		if (m_stKCFTrackerInfos[i].iHandel != 0)
		{
			...
			if (d_lTimeout > 30)		// 30s
			{
				...
				//dzlog_debug ("AntionChan: %02d tracker timeout\n", i);
				continue;
			}
			...
			if (l_fPeakValue < m_fKCFLevel)
			{
				...
				//dzlog_debug ("AntionChan: %02d tracker lost the target, peak=%f\n", i, l_fPeakValue);
			}
			else 
			{
				...
				//dzlog_debug ("AntionChan: %02d tracker update(%d, %d, %f)\n", i, m_stKCFTrackerInfos[i].iCenterX, m_stKCFTrackerInfos[i].iCenterY, l_fPeakValue);
			}
		}
	}
	//dzlog_debug ("<- KCFTrackerUpdate(0)\n");
  ...
  int KCFTrackerEx::iKCFTrackerCheck(int x, int y, int p_iWidth, int p_iHeight)
  {
	  ...
	  int		l_iCenterY = 0;
	  int		l_iStander = 0;
	  //dzlog_debug ("AntionChan: -> iKCFTrackerCheck(%d, %d, %d, %d)\n", x, y, p_iWidth, p_iHeight);
	  ...
	  for (int i = 0; i < KCF_MAX_NUM; i ++)
	  {
		  if (abs(l_iCenterX - m_stKCFTrackerInfos[i].iCenterX) < l_iStander &&
			  ...
			  m_stKCFTrackerInfos[i].iHandel != 0)
			  {
				  l_iIndex = i;
				  //break;
				  //dzlog_debug ("AntionChan: <- iKCFTrackerCheck(1), Target[%d, %d, %d, %d] is in the list\n", x, y, p_iWidth, p_iHeight);
	...
	cout << "l_iTrackerID = " << l_iTrackerID << endl;
	  if (l_iTrackerID >= 0)
	  {
			...
			{
				//dzlog_debug ("AntionChan: What is going on?\n");
				//l_mRoi.release();
				return 0;
			}
	 ...
	 // dzlog_debug ("AntionChan: <- iKCFTrackerCheck(0)\n");
	  //l_mRoi.release();
	  return 0;
}
...
int KCFTrackerEx::iKCFTrackerFind(void)
{
	...
	//dzlog_debug ("AntionChan: <- iKCFTrackerFind(l_iIndex:%d)\n", l_iIndex);
